  ...
  `setup_remote.sh`는 system dependency, `uv`, OpenPI, 현재 패키지, BEHAVIOR-
  1K / OmniGibson 평가 dependency를 설치합니다.
  ...
  `task_checkpoint_mapping.json`을 사용하면 BEHAVIOR task id별로 다른 checkpoint
  를 사용할 수 있습니다.
  ...
  mapping 파일은 `0`부터 `49`까지 모든 task id를 포함해야 합니다.
  ...
  - `compute_norm_stats.py`를 먼저 실행하지 않으면 학습 또는 inference에서 필요
  한 normalization/correlation stats를 찾지 못할 수 있습니다.
  - 기본 config는 `gs://openpi-assets/checkpoints/pi05_base/params`에서 초기
  weight를 읽습니다. 해당 경로를 사용할 수 없다면 `src/b1k/training/config.py`의
  `weight_loader`를 수정해야 합니다.
